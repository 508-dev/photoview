...
RUN set -a && source /env && set +a \
    && go env \
    && CGO_LDFLAGS="${CGO_LDFLAGS:-} -L/usr/local/lib -Wl,-rpath-link,/usr/local/lib -Wl,-l:libheif.so.1 -Wl,-l:libwmflite-0.2.so.7 -Wl,-l:libzip.so.5" go build -v -o photoview .
